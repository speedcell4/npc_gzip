...
if __name__ == '__main__':
    ...
    train_idx_fn = os.path.join(args.output_dir, f'{args.dataset}_train_indicies_{args.num_train}per_class')
    test_idx_fn = os.path.join(args.output_dir, f'{args.dataset}_test_indicies_{args.num_test}per_class')
    ...
    if args.dataset not in ['20News', 'Ohsumed', 'Ohsumed_single', 'R8', 'R52', 'kinnews', 'swahili', 'filipino',
                            'kirnews']:
    ...
    if not args.all_test:
        ...
        if args.test_idx_fn is not None:
            ...
            test_data, test_labels = pick_n_sample_from_each_class_given_dataset(dataset_pair[1], args.num_test,
                                                                                 test_idx_fn)
    else:
    ...
    if not args.record:
        ...
        if not args.score:
            for i in range(0, len(test_data), 100):
                print(f'from {args.test_idx_start + i} to {args.test_idx_start + i + 100}')
                output_rel_fn = f'test_dis_idx_from_{args.test_idx_start + i}_to_{args.test_idx_start + i + 100}'
                output_dir = os.path.join(args.output_dir, os.path.join('distance', args.dataset))
                record_distance(args.compressor, np.array(test_data)[i:i + 100], output_rel_fn, train_data,
                                agg_by_concat_space, NCD, output_dir, para=args.para)
        else:
